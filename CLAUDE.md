...
- **실행 파일은 절대경로로 해석한다** — 로그인 셸에 물어보고(`$SHELL -l -c 'command -v X'`),
  ...
  ⚠️ 이 프로브는 **비대화형**이라 zsh가 `.zshrc`를 건너뛴다. PATH를 `.zshrc`에만 두는(흔한) 구성에선
  1단계가 그냥 실패하므로 **폴백 목록이 장식이 아니라 주 경로다.** `-i`를 더하면 rc가 무거운
  환경에서 앱 시작이 그만큼 늘어져(첫 창 전 메인 스레드에서 도는 호출) 일부러 안 붙였다.
- **사용자 명령은 로그인 + 대화형 셸(`-lic` / `-l -i -c`)로 감싼다** — 안 그러면 `pnpm`·`claude`가
  `command not found`로 즉사한다. **`-i`가 핵심이다**: 로그인만으로는 `.zshenv`·`.zprofile`까지만
  읽는다(gh#2 — 탭만 `-lc`여서 Claude 버튼이 죽었다). 프로브와 달리 여기는 사용자가 연 탭 안에서
  도는 일이라 rc 비용을 져도 된다. 두 자리가 다시 갈리지 않게 테스트로 못 박아 뒀다
  (`TerminalSessionTests.탭과_서비스세션은_같은_셸_조건을_쓴다`).
