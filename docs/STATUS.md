...
- **워크트리 세션 귀속 (M4 후속 · 계획·근거 = [ARCHITECTURE D31](ARCHITECTURE.md) · orca 대조 반영)**: 에이전트에게 "워크트리 만들어 작업해"를 시키면 cc가 옛 탭 안에서 `git worktree add` 후 그 안으로 cd한다 → **워크트리가 사이드바에 안 뜨고 살아있는 cc가 옛 탭에 갇힌다**. orca(`../orca`)는 이걸 **외부 워크트리 인박스 + 영속 baseline**으로 풀며 자동 승격하지 않는다 — muxa는 그 원리만 경량으로. 진행:
  - ✅ **순수 코어**(`WorktreeMembership.swift` + 11 테스트): `WorktreePromotion.pending`(아직 Project 아닌 워크트리) · `WorktreeMove.target`(cwd가 소속과 다른 워크트리 → 이동 대상). `normalizePath` 공통화.
  - **남음** — (1) **감지**: `WorktreeMonitor`(FSEvents 공통 `.git` 감시, 폴링 아님) → worktreeList. **⚠️ 경계 강제(리뷰 I2)**: cwd(OSC 7)와 worktree 경로(git은 real path 반환)의 심링크 표현이 갈리면(`/tmp`↔`/private/tmp` 등) 매칭이 어긋나 offer 오탐·배지 미스 — 모니터가 `resolvingSymlinksInPath`로 **통일해서** 순수 판정에 넘겨야 한다(`normalizePath`는 슬래시만 정규화). (2) **제안(경량)**: 델타 = pending − 영속 baseline(Workspace 필드 1개) → **주의 인박스 상단에 "새 워크트리 추가?" 섹션**(추가=승격 / 무시=baseline 적재). `AttentionEntry`는 안 건드리고 파생 offer로. (3) **이동 배지(경계·육안)**: `WorktreeMove.target` → 칸 상단바 배지 → 클릭 시 서피스 살린 채 소속만 재부모화(**D28 영역 — 블라인드 금지**, `make relaunch` 육안). `cd`로는 소속 불변.
