...
- **워크트리 세션 귀속 (M4 후속 · 계획·근거 = [ARCHITECTURE D31](ARCHITECTURE.md) · orca 대조 반영)**: 에이전트에게 "워크트리 만들어 작업해"를 시키면 cc가 옛 탭 안에서 `git worktree add` 후 그 안으로 cd한다 → **워크트리가 사이드바에 안 뜨고 살아있는 cc가 옛 탭에 갇힌다**. orca(`../orca`)는 이걸 **외부 워크트리 인박스 + 영속 baseline**으로 풀며 자동 승격하지 않는다 — muxa는 그 원리만 경량으로. 진행:
  ...
  - ✅ **스텝 4 링크 탭 (D31 이동 배지의 실현)** — 워크트리에서 도는 작업이 **다른 프로젝트 탭에 갇혔을 때**(cc가 옛 탭에서 worktree add 후 진입) 자동 승격된 프로젝트의 **첫 화면 = 전용 링크 탭**(`TabContent.worktreeLink` + `WorktreeLinkPane` — 셸 스폰 없음, 스냅샷 영속 안 함, 오버레이 배너는 폐기·사용자 피드백). 대상/액션은 스토어 위임(`worktreeLink`/`onWorktreeLinkAction`)으로 AppState가 맡는다. 가져오기 후 링크 탭 자동 닫힘. `WorktreeLink.owner`(순수 임자 판정 — 레포 루트가 하위 워크트리 세션 가로채는 것 방지) + `AppState.externalLiveSession`(다른 store들 OSC7 `pwds` 스캔). **[가서 보기]**=`focusAgentTab`(원본 탭 점프). **[가져오기]**=**영속(∞ tmux) 탭만** — `handOffPersistentTab`(A에서 세션 맵 먼저 떼어 kill/record 없이 detach) → `store.reattach`(B에서 같은 세션 attach). **라이브 서피스 이식 안 함**(프로세스는 tmux 서버에 산다) — cross-store 서피스 수술 회피. 일반 터미널은 가서 보기만(프로세스가 로컬 서피스에 묶임). **★ 육안 미검증**(핵심: 실제 워크트리 add→배너 표시, 가져오기 후 cc 생존, 제거→취소선 배지).
