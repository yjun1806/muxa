...
- **업데이트 확인·자기설치(D39)**: 릴리스가 24h마다 GitHub 태그 API로 최신 태그↔plist 버전 semver 비교(순수 `SemVer`·`UpdateCheck`). 있으면 액티비티 레일 최하단 **배지**(웨글 강조·없으면 렌더 안 함) → 팝오버 "업데이트" → 백그라운드 `git pull && bootstrap && make release-install`(`UpdateInstaller`, 화면 밖·실패만 표면화·자동 재실행 X). 상태=`UpdateChecker`(@Observable) `phase`. 설정에 자동확인 토글 + "지금 확인". dev 스킵. 소스 루트=plist `MUXASourceRoot`→XDG 폴백(muxa 저장소 검증).
...
- **아직 미검증 ★ (이번 세션)** — ① 우측 액티비티 레일 활성 표시를 **뒷배경 강조만**으로(좌측 브랜드 마크바 제거, `ActivityRail`) ② 탭 액션 레인 **Claude 버튼** — 누르면 지속 탭에서 `claude`가 **바로 실행**. *실행은 **tmux 세션의 첫 프로세스로 굽는다**(`startCommand(runCommand:)` = `new-session … '$SHELL -lc claude; exec -l $SHELL'`) — 폴링·지연·프롬프트 타이밍 전무, 로그인 셸로 PATH 복원(빈약 PATH+3중 인용을 실측 통과), 순수 로직은 테스트로 못 박음. send-keys 방식은 폐기(유실·서버 붕괴·`claude%` 이중 렌더). 아이콘 렌더·실제 claude 실행은 육안 필요.*
- **닫기 배너 키보드(IME) 수정** — ∞ 닫기 확인 배너의 ⌘W/⌘B/⌘C가 한글 IME에서 안 먹던 것 keyCode 기반으로 고침(`closeConfirmShortcut`). 육안 확인 필요.
- **업데이트 확인·자기설치(D39)** — 배지 4상태(available 웨글·updating·updated·failed)는 데모 주입으로 **육안 확인됨 ✅**. 하지만 **실제 재빌드 흐름(`make release-install`이 실행 중 `/Applications/muxa.app`를 교체 → 재시작 반영)은 미실행 ★** — 원격에 `0.3.0`보다 높은 태그가 있어야 실검증 가능. 태그 사이 main 커밋은 "최신"으로 표시되는 설계 한계도 실사용 확인 전.
...
- **로컬 모니터 단축키는 keyCode로 판정한다(문자 금지)** = 한글 IME가 켜지면 `charactersIgnoringModifiers`가 자모('ㅈ')를 돌려줘 `"w"` 비교가 빗나간다. ∞ 닫기 확인 배너의 ⌘W/⌘B/⌘C가 그래서 안 먹었다(배너는 keymap=keyCode라 떴지만, 배너 위 결정은 characters라 실패 — 그 비대칭이 증상이었다). `closeConfirmShortcut`을 keyCode로 통일(keymap과 동일). *메뉴 keyEquivalent·`⌘+숫자`·SearchOverlay는 별도 경로라 이미 IME 안전.*
