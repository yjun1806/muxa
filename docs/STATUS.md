...
### ★ 육안 검증 필요 (USAGE-A1 — `make relaunch`)
...
### ★ 육안 검증 필요 (LIST-SPLIT — `make relaunch`)
...
### ★ 육안 검증 필요 (DOCK-REFINE — `make relaunch`)
...
### ★ 육안 검증 필요 (DOCK-TABS — `make relaunch`)
...
### ★ 육안 검증 필요 (SCRIPT-BG — `make relaunch`)
...
연장시켜 아무도 못 빠져나오는 핑퐁. 게다가 개발빌드는 `make relaunch`마다 시작 직후 1회 프로브(idle).
