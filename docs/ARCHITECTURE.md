...
| D40 | **테스트 프레임워크는 swift-testing 하나 — `XCTest` 금지** | `import XCTest` 47개를 swift-testing으로 이전(assert 1,161개)하고, `Makefile`이 CLT 환경에서 `Testing.framework`·`lib_TestingInterop.dylib` 경로를 주입한다(`DEVELOPER_ROOT`→`CLT_DEV`→`TEST_FLAGS`). 규칙은 `CLAUDE.md`에 못 박고 회귀 테스트로 감시한다 | `XCTest`는 **전체 Xcode에만** 있고 Command Line Tools에는 없다. `testTarget`이 `muxaTests` 하나여서 **한 파일만 XCTest를 써도 CLT 환경에서 전체 테스트가 컴파일조차 안 된다** — README·SETUP이 CLT만으로 충분하다고 안내하므로 그 약속이 깨졌다(gh#3). 이전만으로는 부족하다: CLT에 파일은 있지만 두 경로로 흩어져 있고 SwiftPM이 자동으로 안 잡아 `no such module 'Testing'`으로 죽는다(5,330줄을 바꾸기 전 최소 패키지로 확인). 대가 — **swift-testing은 기본 병렬**이라 XCTest의 순차 실행에 기대던 코드가 드러난다(JSContext를 `static`으로 공유하던 24건). 그건 이전이 만든 버그가 아니라 가려져 있던 결함이라 공유 자체를 없앴다. `XCTSkip`은 대응물이 없어 **건너뛰기가 실패로 바뀐다**(리소스를 소스 트리에서 직접 읽으므로 못 읽는 건 실제 문제). 경로 주입은 공식 지원이 아니라 툴체인 갭 우회이므로 배치가 바뀌면 깨질 수 있고, `Makefile`이 조용히 실패하지 않고 무엇이 없는지 말한다 |
