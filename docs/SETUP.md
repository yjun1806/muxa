...
make app          # 2. .app 번들로 빌드·실행 (권장 — 아이콘·시스템 알림 정상)
```
...
make run          # bare 실행 (프로세스명 muxa-dev-<slug>) — 창은 뜨지만 알림·아이콘은 번들 전용
make test         # 순수 로직 단위 테스트
```

## 앱 번들(.app)로 빌드

`make run`(= bare `.build/debug/…` 실행)은 창은 뜨지만 **번들 id가 없어 Finder/Dock 아이콘이 없고 시스템 알림이 Dock 바운스로 폴백**한다. 정식 아이콘·시스템 알림을 쓰려면 `.app` 번들로 빌드한다.

```bash
make app          # debug 번들 빌드·실행 (개발)
make install      # release 번들 → /Applications 설치 (프로덕션, 재시작해야 반영)
```

`make app`은 `scripts/build-app.sh`를 부른다. SPM은 실행파일만 내므로 이 스크립트가 `Info.plist`·`AppIcon.icns`·SPM 리소스 번들을 `.app` 구조로 조립하고 ad-hoc 서명한다.
...
`<slug>`는 워크트리 디렉터리 이름(메인 체크아웃이면 브랜치명). 여러 muxa를 동시에 띄워도 안 헷갈리고, **엉뚱한 인스턴스를 죽이지 않는다**. 이 워크트리 것만 확인·종료하려면 `make whoami` / `make kill`(릴리스·타 워크트리 불가침). 앱 아이콘을 바꾸려면 `scripts/build-appicon/`(Core Graphics 드로잉, SVG 래스터라이저 의존 없음)을 고치고 `make icons`를 돌린다 — `macos/AppIcon.icns` + `Resources/AppIcon.png`(런타임 Dock 아이콘)가 재생성된다.
...
muxa는 에이전트(Claude Code 등)와 매끄럽게 물리는 세 기능을 갖췄지만, 셋 다 사용자 환경 설정이 있어야 켜진다. `make integrate`(= `scripts/install-integration.sh`)가 이 세 관문을 한 번에 처리한다.
