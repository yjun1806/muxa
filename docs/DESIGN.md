...
2. **레이아웃·워크스페이스 상태는 Swift가 소유하고 SQLite에 영속한다.** 분할 트리는 순수 값 타입(`tree.ts` 이식) + 단위 테스트.
...
- **패인당** GhosttyKit 서피스 하나 — PTY·VT 상태·스크롤백·GPU 렌더를 엔진이 소유. 앱은 `SurfaceView`(NSView)로 감싸 분할 트리에 배치
- 한글 IME: `SurfaceView`가 NSTextInputClient 구현(D15) — 조합 미리보기(marked text)가 커서 위치에 인라인 표시
- 백그라운드 세션: 서피스는 살아있되 화면 밖 — 렌더 비용은 보이는 서피스만. 메모리 상한은 ghostty 스크롤백 설정으로 제어
- **검색(Find)**: 에이전트 출력을 되짚는 본질 동선. GhosttyKit 노출 API 확인 후, 없으면 스크롤백 덤프 기반 자체 구현(M1에서 확정)
...
| M1 | 터미널 코어 | 워크스페이스 + **재귀 분할 트리**(`tree.ts` 이식·단위 테스트) + 설정 시스템(ghostty config 재사용 검토) + 터미널 Find + 분할 트리 직렬화 세션 복구 |
...
| 자산 | 처분 |
|---|---|
| `src/tree.ts` (분할 트리 순수 로직) | **Swift 이식** — 설계 그대로, 테스트 포함 |
