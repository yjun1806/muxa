...
화면 영역이 많아 이름을 확정한다. **이 표가 단일 진실 원천**이다. 새 코드·문서·UI 라벨은 이 용어를 쓴다.

| 한국어 | English | 무엇 |
|--------|---------|------|
| **상단바** | Top Bar | 맨 위 줄 — 워크스페이스·프로젝트 탭, 활성 경로, 탐색기/Git 토글 |
...
| **서비스 도크** | Service Dock | 하단 **오버레이**(⌘J) — 좌: 서비스 목록, 우: 로그. 카드를 리사이즈하지 않는다 |
| **서비스** | Service | 프로젝트에 속한 **장수 프로세스**(dev 서버 등). 탭 트리 밖, tmux가 실행(D19) |

**용어 구분·주의**
- **칸(Pane) ≠ 뷰어(Viewer)**: 칸은 분할 단위, 뷰어는 파일을 그리는 콘텐츠(코드 뷰어·문서 뷰어). "뷰"는 이 둘·`TermView`(SwiftUI View)와 3중 충돌해 **쓰지 않는다**.
- **메인 영역(Main Area) ≠ 워크스페이스**: VSCode 한국어판이 Workspace를 "작업 영역"으로 번역하므로 최상위 계층(워크스페이스)과 혼동을 피하려 "작업 영역"을 쓰지 않는다.
- **도구 패널(우) ≠ 사이드바(좌)**: 말이 겹치는 "사이드 패널"을 피한다.
- 계층: **워크스페이스 › 프로젝트 › 칸 › 탭 › 서브탭**.

> 아래 다이어그램은 초기 스케치라 일부 용어가 옛것(패인 등). 이름은 위 표를 따른다.
...
### 컬러 토큰 (SSOT = `Palette.swift`)

서비스 **키 컬러는 teal**(= 앱 아이콘 배경색). 강조·포커스·선택·CTA는 전부 이 키에서 파생하고, 색은 `Palette.swift` 한 곳에만 둔다(하드코딩 금지). 라이트/다크는 동적 `NSColor`로 자동 전환.

**브랜드 스케일** (`Palette.Brand`, Tailwind teal 계열의 지각 균등 스케일):

| 토큰 | hex | 비고 |
|------|-----|------|
| `Brand.s50`~`s300` | F0FDFA·CCFBF1·99F6E4·5EEAD4 | 옅은 틴트(배경·hover) |
| **`Brand.key` (= s400)** | **2DD4BF** | **서비스 키 컬러 — 아이콘 배경** |
| `Brand.s500`~`s900` | 14B8A6·0D9488·0F766E·115E59·134E4A | 진한 강조(라이트 대비·음영) |
| `Brand.gradTop`/`gradBottom` | 35DECA / 27C4B1 | 아이콘 배경 그라디언트(키 주변 명/암) |

**브랜드 semantic** (키에서 파생, 라이트/다크):
...
| `onBrand` | 딥 차콜 0A2E2A | 키 컬러 위 전경(텍스트·아이콘) |

**기능색(브랜드와 별개, 의미 고정)**: 활동 대기 = 주황(`borderActivity`), git added = 초록·modified = 노랑·deleted = 빨강·renamed = 파랑·conflict = 주황, PR merged = 보라. 이들은 관례적 의미색이라 키 컬러에 종속시키지 않는다.
