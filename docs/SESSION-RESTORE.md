...
설치한다(`integrate.sh`). OSC 133 `A`(프롬프트 시작)를 한 번 받으면 그때 보낸다.
...
아이러니하게 우리 `integrate.sh`는 **아직도 그 스니펫을 심고 있는데**(env를 더 이상
...
**opt-in의 형태가 바뀌었다: 설정이 아니라 버튼이다.**

당초 `[session] persistent = false` 설정으로 켜려 했으나 폐기했다. 설정이 켜져 있으면 `+`와 `∞`가
...
|---|---|
| `+` 버튼 / ⌘T | 일반 터미널 |
| `∞` 버튼 | tmux 지속 세션 (tmux가 있을 때만 노출) |
| 분할 | **원본 칸에서 물려받는다** (cwd를 물려받는 것과 같은 규칙) |
| 탭 아이콘 | 지속 세션은 `∞` — 어느 탭이 tmux 안인지 모르면 "닫아도 되나"를 판단할 수 없다 |

**탭을 닫을 때 (C 정책)**

무조건 죽이면 ⌘W를 잘못 눌렀을 때 30분 돌던 빌드가 즉사한다(tmux를 쓰는 이유를 반쯤 버린다).
무조건 남기면 눈에 안 보이는 유령이 쌓인다. 그래서 **안에서 작업이 돌고 있을 때만 남긴다**.

- 판정: **프로세스 트리를 훑는다.** tmux의 `#{pane_current_command}`는 pane의 맨 위만 보고,
  TTY 포그라운드는 래퍼가 자체 pty를 만들면 안 뚫린다. 사용자 셸이 `kiro-cli-term` 같은 래퍼로
  감싸져 있으면 안에서 빌드가 돌아도 영원히 "zsh"라 답해 판정이 영영 안 걸린다(두 번 걸렸다).
- 남긴 세션은 `Project.detached`에 기록한다. 기록 없이 남기면 ① 다음 시작 때 GC가 고아로 보고
  죽이고 ② 사용자가 존재를 모른다.
- 복구: 푸터의 **"백그라운드 N" 칩** → 목록 → **열기**(그 세션에 다시 붙어 화면·프로세스가 그대로
  돌아온다) 또는 **종료**. 인박스에도 한 줄 남긴다.

**실기기 검증 완료**: `sleep 900` 실행 → 앱 종료 → 재시작 → 프로세스 생존 + 같은 세션 재부착 +
화면 보존. cwd 추적(OSC 7)과 훅 알림도 tmux 안에서 동작한다(§5.3).

**빠지면 조용히 깨지는 것들** (전부 테스트로 고정):
- `remain-on-exit off` — 서버 전역은 `on`이다(서비스가 죽은 뒤 로그를 읽어야 하므로). 터미널에
  그대로 두면 `exit`를 쳐도 pane이 죽은 채 남아 탭이 안 닫힌다.
- `-e`로 env 주입 — tmux 세션의 셸은 tmux **서버**의 환경을 상속한다. MUXA_TAB_ID가 없으면
  훅 알림이 어느 탭인지 못 찾고 rc 스니펫도 안 돈다.
- 훅 라우팅 역매핑 — `-e`는 세션을 새로 만들 때만 적용된다. 복원하면 tabId가 새로 발급되는데
  세션 안 셸의 env에는 **처음 tabId**가 박혀 있다. 세션명이 다리가 된다.
- 세션명을 스냅샷에 저장 + **발급 즉시** — 렌더 시점에 정해지는데 그때 persist하지 않으면
  다음 실행에서 새 세션을 만들고 옛 세션은 고아로 죽는다(그 안의 빌드와 함께).
- 고아 GC 분리 — 서비스 GC가 터미널 세션을 죽이지 않고, 터미널 GC도 다른 muxa 인스턴스의
  세션을 건드리지 않는다(같은 tmux 소켓을 공유한다).
...
3. **trusted 재정의가 기존 흐름을 바꾼다.** 지금 "claude면 자동 재개"에 익숙한 상태에서,
   훅이 없으면 갑자기 버튼을 눌러야 한다. 훅 설치 안내(`integrate.sh`)가 확실히
   동작하는지가 전제다.
...
- `scripts/integrate.sh:202`가 심는 rc 스니펫은 현재 no-op이다(env 미주입).
