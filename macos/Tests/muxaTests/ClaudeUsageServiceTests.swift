...
struct ClaudeUsageServiceTests {
    ...
    @Test func testFreshStatusLineSkipsApi() async {
        ...
        #expect(clock.calls == 0, "A-1이 신선하면 fetcher를 부르지 않는다")
    ...
    @Test func testStaleStatusLineFallsBackToApi() async {
        ...
        await service.refreshIfStale()
        #expect(clock.calls == 1, "A-1이 낡으면 A-2로 조회한다")
    }
    ...
    @Test func testFreshStatusLineMergesCachedFable() async {
        ...
        await service.refreshIfStale()
        #expect(clock.calls == 0, "A-1이 신선하면 API를 건드리지 않는다")
        #expect(service.limits.contains { $0.kind == "session" }, "A-1의 세션 한도")
        #expect(service.limits.contains { $0.isModelScoped }, "A-2 캐시의 Fable이 병합됨")
    }
    ...
    @Test func testFreshStatusLineRefreshesStaleFable() async {
        ...
        await service.refreshIfStale()
        #expect(clock.calls == 1, "Fable이 낡았으면 A-2를 한 번 태운다")
        #expect(service.limits.contains { $0.kind == "session" }, "A-1의 세션 한도는 그대로")
        #expect(service.limits.first { $0.isModelScoped }?.percent == 42, "Fable이 새 값으로 갱신됨")
    }
    ...
    @Test func testFreshStatusLineWithFreshFableSkipsApi() async {
        ...
        let service = makeService(clock, results: [.ok([])], store: store, statusLine: { a1 })
        await service.refreshIfStale()
        #expect(clock.calls == 0, "Fable이 아직 신선하면 A-2를 건드리지 않는다")
    }
    ...
    @Test func testManualRefreshBypassesCacheTTL() async {
        ...
        #expect(did)
        #expect(clock.calls == 2, "수동은 캐시 TTL을 우회한다")
    }
    ...
    @Test func testManualRefreshHonorsRateLimitBackoff() async {
        ...
        await service.refreshIfStale()
        #expect(service.state == .rateLimited)
    ...
    @Test func testSuccessIsCachedForFifteenMinutes() async {
        ...
        await service.refreshIfStale()
        #expect(clock.calls == 1, "15분 전엔 재조회하지 않아야 한다")

        clock.now.addTimeInterval(2)
        await service.refreshIfStale()
        #expect(clock.calls == 2, "15분이 지나면 재조회한다")
    }
    ...
    @Test func testFailureRetriesSoonerThanSuccess() async {
        ...
        await service.refreshIfStale()
        #expect(clock.calls == 1, "첫 실패 백오프(120초) 전엔 재시도하지 않는다")

        clock.now.addTimeInterval(2)
        await service.refreshIfStale()
        #expect(clock.calls == 2, "120초가 지나면 재시도한다")
        #expect(service.state == .ok)
        #expect(service.limits == sample)
    }
    ...
    @Test func testConsecutiveFailuresBackOffExponentially() async {
        ...
        await service.refreshIfStale()
        #expect(clock.calls == 2, "2연속 실패면 백오프가 240초로 늘어 121초엔 재시도하지 않는다")

        clock.now.addTimeInterval(120)            // 누적 241초 → 240 넘김
        await service.refreshIfStale()
        #expect(clock.calls == 3, "240초가 지나면 재시도한다")
        #expect(service.state == .ok)
    }
    ...
    @Test func testBusyDefersReprobeUntilBusyInterval() async {
        ...
        #expect(clock.calls == 1, "라이브 세션 중엔 15분이 지나도 재조회를 미룬다")

        clock.now.addTimeInterval(900)            // 누적 1801초 → busyInterval(1800) 넘김
        await service.refreshIfStale(busy: true)
        #expect(clock.calls == 2, "그래도 30분 상한에선 재조회한다 — 영영 굳지 않게")
    }
    ...
    @Test func testFailureKeepsPreviousLimits() async {
        ...
        #expect(service.state == .failed)
        #expect(service.limits == sample, "이전 조회 결과가 유지돼야 한다")
        #expect(service.lastSuccess == Date(timeIntervalSince1970: 1_000_000), "성공 시각은 실패로 갱신되지 않는다")
    }
    ...
    @Test func testRateLimitHonorsRetryAfter() async {
        ...
        await service.refreshIfStale()
        #expect(clock.calls == 1, "실패 백오프(120초)로 재시도하면 안 된다 — 리밋이 연장된다")

        clock.now.addTimeInterval(3000)
        await service.refreshIfStale()
        #expect(clock.calls == 1, "retry-after(3068초) 전엔 재시도하지 않는다")

        clock.now.addTimeInterval(30)
        await service.refreshIfStale()
        #expect(clock.calls == 2, "retry-after가 지나면 재시도한다")
        #expect(service.state == .ok)
    }
    ...
    @Test func testRateLimitWithoutHeaderWaitsDefault() async {
        ...
        clock.now.addTimeInterval(899)
        await service.refreshIfStale()
        #expect(clock.calls == 1, "기본 대기(15분) 전엔 재시도하지 않는다")
    ...
    @Test func testRateLimitWaitIsCappedAtOneHour() async {
        ...
        await service.refreshIfStale()
        #expect(clock.calls == 2, "1시간이 지나면 상한에 걸려 재시도한다")
    }
    ...
    @Test func testSharedCacheServedToOtherInstance() async {
        ...
        #expect(clockB.calls == 0, "A의 신선한 캐시가 있으면 B는 네트워크를 건드리지 않는다")
        #expect(serviceB.state == .ok)
        #expect(serviceB.limits == sample, "B는 공유 캐시 값을 그대로 보여준다")
    }
    ...
    @Test func testRateLimitBackoffSharedAcrossInstances() async {
        ...
        await serviceA.refreshIfStale()
        #expect(serviceA.state == .rateLimited)

        let clockB = Clock() // t0 — A의 백오프 창(1800초) 한참 안쪽
        let serviceB = makeService(clockB, results: [.ok(sample)], store: store)
        await serviceB.refreshIfStale()
        #expect(clockB.calls == 0, "A가 429 백오프 중이면 B는 프로브하지 않는다 — 이게 리밋 연장을 막는다")
        #expect(serviceB.state == .rateLimited, "B도 제한 상태로 표시(로그인 문제가 아니다)")
    }
    ...
    @Test func testFreshInstanceShowsCachedLimitsWhileRateLimited() async {
        ...
        #expect(service.state == .rateLimited)
        #expect(service.limits == sample, "제한 중에도 공유 캐시의 이전 값을 보여준다")
    }
}
