...
struct CommandStoreTests {
    ...
    @Test func testRecordStartMergesSameCommand() {
        ...
        #expect(e[0].executions.map(\.id) == ["2", "1"], "최신이 앞")
        #expect(e[0].cwd == "/y", "cwd 갱신")
        #expect(e[0].runCount == 2)
    }
    ...
    @Test func testExecLimitDropsOldest() {
        ...
        #expect(e2[0].executions.count == CommandStore.execLimit, "상한 유지")
        #expect(dropped == ["e0"], "가장 오래된 e0이 밀려남(로그 삭제 대상)")
        #expect(e2[0].executions.first?.id == "new")
    }
    ...
    @Test func testToggleFavoriteMovesSections() {
        ...
        #expect(s.favorites.map(\.command) == ["a"])
    ...
    @Test func testToggleFavoriteCreatesForDiscovered() {
        ...
        #expect(e[0].favorite)
        #expect(e[0].name == "dev")
        #expect(e[0].executions.isEmpty, "아직 실행 전이므로 내역 없음")
    }
    ...
    @Test func testPanelSectionsDedup() {
        ...
        #expect(s.favorites.map(\.command) == ["pnpm dev"], "즐겨찾기")
        #expect(s.projectScripts.map(\.command) == ["pnpm build"], "발견 중 즐겨찾기 아닌 것만(dev는 빠짐)")
        #expect(s.history.map(\.command) == ["brew install jq"], "발견도 즐겨찾기도 아닌 즉석만")
    }
    ...
    @Test func testDiscoveredRunAppearsInCatalogAndHistory() {
        ...
        #expect(s.history.map(\.command) == ["pnpm dev"], "실행했으니 히스토리에도")
        _ = e
    }
    ...
    @Test func testMigrate() {
        ...
        #expect(m.first { $0.command == "pnpm dev" }?.favorite == true, "등록→즐겨찾기")
        #expect(m.first { $0.command == "pnpm dev" }?.name == "web", "이름 보존")
        #expect(m.first { $0.command == "pnpm test" }?.favorite == false, "히스토리→비즐겨찾기")
        #expect(CommandStore.migrate(scripts: [], history: []) == nil, "둘 다 비면 nil")
    }
    ...
    @Test func testSetCwdAndRemove() {
        ...
        #expect(e2.isEmpty)
        #expect(dropped == ["1"], "지운 명령의 실행 로그도 정리 대상")
    }
}
