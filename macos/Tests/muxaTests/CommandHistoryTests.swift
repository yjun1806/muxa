...
struct CommandHistoryTests {
    ...
    @Test func testRecordMergesDuplicateToFront() {
        ...
        #expect(h.map(\.command) == ["a", "b"], "재실행한 a가 맨 앞으로")
        #expect(h.first?.runCount == 2, "runCount 누적")
        #expect(h.first?.lastRunAt == at(2), "lastRunAt 갱신")
    }
    ...
    @Test func testRecordCapsAtLimit() {
        ...
        #expect(h.count == CommandHistory.limit)
        h = CommandHistory.record(h, command: "new", name: "new", cwd: nil, now: at(1000))
        #expect(h.count == CommandHistory.limit, "상한 유지")
        #expect(h.first?.command == "new", "새 것은 맨 앞")
        #expect(!(h.contains { $0.command == "c0" }), "가장 오래된 c0이 밀려남")
    }
    ...
    @Test func testSectionsSplitRegisteredAndHistory() {
        ...
        #expect(reg.count == 1)
        #expect(reg.first?.script.name == "build")
        #expect(reg.first?.lastRunAt == at(2), "등록 명령의 lastRun을 이력에서 가져옴")
        #expect(hist.map(\.command) == ["pnpm i"], "등록된 make build는 히스토리에서 빠지고 미등록만 남음")
    }
    ...
    @Test func testRunStateMatchesLatestInstance() {
        ...
        #expect(st?.scriptId == "i2", "startedAt이 가장 최근인 i2")
    }
