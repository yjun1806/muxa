...
struct AgentChangeSetTests {
    ...
    @Test func 예전_파일도_읽힌다() throws {
        ...
        let set = try JSONDecoder().decode(AgentChangeSet.self, from: Data(raw.utf8))
        #expect(set.entries.count == 1)
    ...
    @Test func 빈_객체도_읽힌다() throws {
        let set = try JSONDecoder().decode(AgentChangeSet.self, from: Data("{}".utf8))
        #expect(set.entries.isEmpty)
        #expect(set.references.isEmpty)
    }
