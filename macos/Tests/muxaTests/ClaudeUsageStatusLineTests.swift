...
struct ClaudeUsageStatusLineTests {
    ...
    @Test func parsesBothWindows() {
        ...
        #expect(session?.label == "5h")
        #expect(session?.percent == 24) // 23.5 반올림
        #expect(session?.resetsAt == Date(timeIntervalSince1970: 1770000000))
        #expect(!(session?.isModelScoped ?? true))
        #expect(limits.first { $0.kind == "weekly_all" }?.label == "wk")
    }
    ...
    @Test func missingWindowIsOmitted() {
        ...
        """)
        #expect(limits.map(\.kind) == ["session"])
    }
    ...
    @Test func windowWithoutPercentIsDropped() {
        ...
        """)
        #expect(limits.map(\.kind) == ["weekly_all"])
    }
    ...
    @Test func windowWithoutResetKeepsPercent() {
        ...
        """)
        #expect(limits.first?.percent == 55)
