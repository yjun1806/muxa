...
struct IdeProtocolTests {
    ...
    @Test func acceptKeyMatchesRfc6455Vector() {
        // RFC 6455 §1.3 표준 예시.
    ...
    @Test func nilExpectedTokenSkipsAuth() {
        #expect(IdeWsHandshake.rejectReason(upgrade(auth: nil), expectedToken: nil) == nil)
    }

    @Test func subprotocolEchoedWhenMcpOffered() {
    ...
    @Test func maskedClientFrameIsUnmasked() {
    ...
    @Test func incompleteFrameReported() {
        if case .incomplete = IdeWsFrame.decode(Data([0x81])) {} else { Issue.record() }
    }

    @Test func hugeDeclaredLengthIsError() {
    ...
    @Test func extendedLength126RoundTrips() {
    ...
    @Test func parseRequestKeepsIntId() {
    ...
    @Test func initializeResultLoggingIsEmptyObject() {
    ...
    @Test func toolsListAdvertisesOnlySupportedTools() {
    ...
    @Test func selectionTextNilIsFailure() {
        #expect(IdeProtocol.selectionText(nil).contains("\"success\":false"))
    }

    @Test func selectionTextEncodesRange() {
    ...
    @Test func lockfileContentFields() {
    ...
    @Test func docSelectionEmptyTextIsEmptySelection() {
        let sel = DocSelectionBridge.selection(from: ["text": ""], filePath: "/a")
        #expect(sel!.isEmpty)
    }

    @Test func docSelectionNonDictReturnsNil() {
