...
struct ClaudeHookSettingsTests {
    ...
    @Test func mergePreservesUnrelatedKeys() throws {
        ...
        #expect(merged["permissions"] != nil, "hooks 밖의 키를 건드리면 안 된다")
    }
    ...
    @Test func mergeReplacesStaleMuxaPath() throws {
        ...
        #expect(stop.count == 1)
        #expect(stop[0] == ClaudeHookSettings.command(for: .stop, executable: exe))
    ...
    @Test func removeKeepsUserHooksAndDropsMuxa() throws {
        ...
        #expect(!(ClaudeHookSettings.isInstalled(in: removed, executable: exe)))
    }
    ...
    @Test func mergeReplacesLegacyStateStyleHooks() throws {
        ...
        #expect(stop.count == 1, "레거시 훅이 남아 이중 발화한다")
        #expect(!(stop[0].contains("--state")), "레거시 형식이 남았다")
        #expect(commands(merged, event: .notification).filter { $0.contains("--state") }.count == 0)
    }
    ...
    @Test func commandIsValidShellSyntax() throws {
        for event in ClaudeHookEvent.allCases {
            ...
            #expect(process.terminationStatus == 0, "셸 문법 오류: \(command)")
        }
    }
    ...
    @Test func mergeReplacesUnquotedBrokenHooks() throws {
        ...
        let stop = commands(merged, event: .stop)
        #expect(stop.count == 1, "깨진 훅이 남아 중복됐다")
    ...
    @Test func nonCommandHooksArePreserved() throws {
        ...
        #expect(types.contains("http"), "command가 아닌 훅이 사라졌다")
        #expect(types.contains("command"), "muxa 훅이 등록되지 않았다")
    }
}
