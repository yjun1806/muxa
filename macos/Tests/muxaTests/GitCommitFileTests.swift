...
struct GitCommitFileTests {
    ...
    @Test func copyBehavesLikeRename() {
    ...
    @Test func mergeCombinedStatusTakesFirstChar() {
    ...
    @Test func numstatBinaryIsUnknownNotZero() {
    ...
    @Test func mergeJoinsRenameAcrossCompressedPath() {
        let files = GitCommitFileParse.merge(
    ...
    @Test func mergeAsymmetryDoesNotMisalign() {
        let files = GitCommitFileParse.merge(
    ...
    /// 짝이 없으면 0을 지어내지 않는다.
    @Test func mergeKeepsStatsNilWhenUnmatched() {
    ...
    @Test func mergeCarriesBinaryFlag() {
        let files = GitCommitFileParse.merge(
