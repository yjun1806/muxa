...
///
struct DocDiffCoreTests {

    /// markdown-it + dmp + core.js를 얹은 컨텍스트. **테스트마다 새로 만든다.**
    ///
    /// 예전에는 `static` 하나를 공유하고 입력을 `__old`/`__new` 전역에 넣었는데, swift-testing은
    /// 기본이 병렬이라 테스트끼리 서로의 입력을 덮어썼다(XCTest는 순차라 드러나지 않았다).
    /// `.serialized`로 덮을 수도 있지만 그건 공유 상태를 남겨둔 채 순서에만 기대는 것이라,
    /// 컨텍스트 자체를 독립시켰다 — 실측 비용은 40개 테스트에 수십 ms 수준이다.
    ...
    @Test func jscEnvironmentIsUsable() throws {
        let c = try ctx()
    ...
    @Test func identicalDocumentHasNoChanges() throws {
        let src = "# 제목\n\n첫 문단입니다.\n\n둘째 문단입니다.\n"
    ...
    @Test func insertedParagraphIsDetected() throws {
        let r = try diff("# 제목\n\n첫 문단.\n", "# 제목\n\n첫 문단.\n\n새 문단.\n")
        #expect(stats(r)["inserted"] == 1)
        #expect(stats(r)["deleted"] == 0)
    }

    @Test func deletedParagraphKeepsItsPlace() throws {
        let r = try diff("# 제목\n\nA 문단.\n\nB 문단.\n", "# 제목\n\nB 문단.\n")
    ...
    @Test func softWrapChangeIsNotAChange() throws {
    ...
    @Test func listMarkerChangeIsNotAChange() throws {
    ...
    @Test func listItemAdditionOnlyMarksThatItem() throws {
        let r = try diff("- 하나\n- 둘\n", "- 하나\n- 하나 반\n- 둘\n")
        #expect(stats(r)["inserted"] == 1)
        #expect(stats(r)["deleted"] == 0)
    ...
    @Test func koreanParticleChangeHighlightsOnlyParticle() throws {
    ...
    @Test func koreanEndingChangeKeepsCommonParts() throws {
    ...
    @Test func koreanSegmentation() throws {
        let c = try ctx()
    ...
    @Test func heavilyRewrittenParagraphBecomesWholeReplacement() throws {
    ...
    @Test func shortSentenceKeepsInlineHighlight() throws {
    ...
    @Test func longParagraphWithSmallEditStaysInline() throws {
    ...
    @Test func codeBlockIsLineDiffedNotWordDiffed() throws {
        let a = "```swift\nlet x = 1\n```\n"
        let b = "```swift\nlet x = 2\n```\n"
        let r = try diff(a, b)
        let mod = blocks(r).first { $0["kind"] as? String == "modified" }
    ...
    @Test func codeBlockMarksChangedLines() throws {
        let a = "```swift\nlet x = 1\nlet y = 2\nprint(x)\n```\n"
        let b = "```swift\nlet x = 1\nlet y = 99\nprint(x)\n```\n"
        let r = try diff(a, b)
    ...
    @Test func tableMarksChangedCell() throws {
        let a = "| 항목 | 값 |\n|---|---|\n| 하나 | 1 |\n| 둘 | 2 |\n"
        let b = "| 항목 | 값 |\n|---|---|\n| 하나 | 1 |\n| 둘 | 22 |\n"
        let r = try diff(a, b)
    ...
    @Test func tableStructureChangeFallsBackToWhole() throws {
        let a = "| A | B |\n|---|---|\n| 1 | 2 |\n"
        let b = "| A | B | C |\n|---|---|---|\n| 1 | 2 | 3 |\n"
        let r = try diff(a, b)
        let table = blocks(r).first { ($0["type"] as? String) == "table" }
    ...
    @Test func htmlBlockFallsBackToWholeChange() throws {
    ...
    @Test func codeFenceLanguageIsPartOfIdentity() throws {
        let r = try diff("```js\nx\n```\n", "```ts\nx\n```\n")
        let kinds = blocks(r).map { $0["kind"] as? String ?? "" }
    ...
    @Test func listItemsCarryListIdentity() throws {
        let r = try diff("- 하나\n- 둘\n", "- 하나\n- 하나 반\n- 둘\n")
    ...
    @Test func orderedListCarriesOlTag() throws {
        let r = try diff("1. 하나\n2. 둘\n", "1. 하나\n2. 하나 반\n3. 둘\n")
        let items = blocks(r).filter { ($0["listId"] as? String) != nil }
    ...
    @Test func separateListsGetDistinctIds() throws {
        let src = "- A\n\n문단.\n\n- B\n"
    ...
    @Test func blockquoteCountsInnerParagraphOnce() throws {
    ...
    @Test func tableIsOneAtomicBlock() throws {
        let a = "| 항목 | 값 |\n|---|---|\n| 하나 | 1 |\n| 둘 | 2 |\n"
        let b = "| 항목 | 값 |\n|---|---|\n| 하나 | 1 |\n| 둘 | 22 |\n"
        let r = try diff(a, b)
    ...
    @Test func tableRowAdditionKeepsSingleBlock() throws {
        let a = "| A | B |\n|---|---|\n| 1 | 2 |\n"
        let b = "| A | B |\n|---|---|\n| 1 | 2 |\n| 3 | 4 |\n"
        let r = try diff(a, b)
    ...
    @Test func creationIsAllInserted() throws {
        let r = try diff("", "# 새 문서\n\n내용.\n")
        #expect(stats(r)["deleted"] == 0)
        #expect(stats(r)["inserted"] ?? 0 > 0)
    }

    @Test func deletionIsAllDeleted() throws {
        let r = try diff("# 옛 문서\n\n내용.\n", "")
        #expect(stats(r)["inserted"] == 0)
        #expect(stats(r)["deleted"] ?? 0 > 0)
    }

    @Test func emptyToEmptyIsNoop() throws {
    ...
    @Test func projectionMatchesRenderedText() throws {
        let c = try ctx()
        c.evaluateScript("""
    ...
    @Test func spansStayInsideProjectedText() throws {
    ...
    @Test func formattingOnlyChangeIsNotSwallowed() throws {
    ...
    @Test func tableCellSpansUseProjectedOffsets() throws {
        let a = "| 항목 | 값 |\n|---|---|\n| 하나 | **굵은값** 뒤 |\n"
        let b = "| 항목 | 값 |\n|---|---|\n| 하나 | **굵은값** 앞 |\n"
    ...
    @Test func tableCellFormattingOnlyChangeIsNotSwallowed() throws {
        let a = "| 항목 | 값 |\n|---|---|\n| 하나 | 강조 |\n"
        let b = "| 항목 | 값 |\n|---|---|\n| 하나 | **강조** |\n"
    ...
    @Test func shiftPastDeletionsMovesSpanPastRestoredText() throws {
        let c = try ctx()
    ...
    @Test func shiftPastDeletionsDoesNotSwallowTrailingDeletion() throws {
        let c = try ctx()
        let v = c.evaluateScript("""
        JSON.stringify(DiffDocPaint._shiftPastDeletions(
    ...
    @Test func shiftPastDeletionsAccumulates() throws {
        let c = try ctx()
        let v = c.evaluateScript("""
        JSON.stringify(DiffDocPaint._shiftPastDeletions(
    ...
    @Test func shiftPastDeletionsDoesNotMutateInput() throws {
        let c = try ctx()
        let v = c.evaluateScript("""
        (function () {
    ...
    @Test func emojiDoesNotBreakOffsets() throws {
        let r = try diff("안녕 🎉 반가워\n", "안녕 🎉 반갑다\n")
        let mod = blocks(r).first { $0["kind"] as? String == "modified" }
