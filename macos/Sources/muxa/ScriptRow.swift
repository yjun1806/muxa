...
/// 서비스 도크 **일회용 탭**의 실행 기록 한 줄 — `ScriptDockRow`와 같은 사각형 축이되,
/// **제목이 명령(mono)**이다. 등록 스크립트는 친근한 이름이 주인공이고 일회용은 날 명령이 주인공 —
/// 이 대비 자체가 "임시로 친 것"을 말한다(별도 배지 없이). hover = 재실행 · 등록(승격) · 기록 삭제.
struct OneOffRow: View {
    let script: Script
    let run: ScriptRun?
    /// 경과 계산 기준 — 부모(일회용 탭)가 tick으로 갱신해 내려준다.
    let now: Date
    var selected = false
    let action: () -> Void      // 클릭 = 상세 선택
    let onRun: () -> Void       // 다시 실행(같은 명령을 새 기록으로)
    let onPromote: () -> Void   // 스크립트로 등록(승격 — 자주 쓰면)
    let onDelete: () -> Void    // 이 기록만 삭제
    ...
    var body: some View {
        HStack(spacing: 0) {
            Button(action: action) {
                HStack(spacing: Space.sm) {
                    Image(systemName: ScriptStatusStyle.glyph(run?.state))
                        ...
                        .foregroundStyle(ScriptStatusStyle.color(run?.state))
                        .frame(width: IconSize.statusSlot)
                    // 명령이 주인공 — `.tail`(가운데 접으면 `&& curl … | sh` 꼬리가 숨는다, ServiceRow 규칙).
                    Text(script.command)
                        .font(.muxaMono(.label))
                        .foregroundStyle(Color.pFg)
                        .lineLimit(1)
            ...
            .buttonStyle(.plain)
            .clickCursor()
            .accessibilityRow(label: "\(script.command), \(ScriptStatusStyle.label(run?.state))",
                              selected: selected)

            // 도는 중엔 재실행·삭제를 감춘다(재실행은 dedup, 삭제는 도는 프로세스를 죽이는 파괴적 동작).
            if run?.isRunning != true {
                FooterAction(icon: "arrow.clockwise", help: "다시 실행", action: onRun)
                    .opacity(hovered ? 1 : 0)
                FooterAction(icon: "plus.square", help: "스크립트로 등록", action: onPromote)
                    .opacity(hovered ? 1 : 0)
                FooterAction(icon: "trash", help: "기록 삭제", destructive: true, action: onDelete)
                    .opacity(hovered ? 1 : 0)
            }
        }
        .padding(.horizontal, Space.sm)
