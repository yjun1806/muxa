...
/// 푸터의 스크립트 칩 — 등록된 스크립트(끝이 있는 명령)의 실행 상태 요약 + 도크 진입점.
///
/// **서비스 칩과 대칭**이 됐다: 클릭하면 팝오버가 아니라 **서비스 도크의 스크립트 탭**이 열린다
/// (목록·실행·추가·삭제가 전부 도크에 있어 중간 팝오버가 필요 없다 — `ServiceStrip.toggleDock`과 같은 계약).
/// 등록 0개여도 상시로 그려 첫 등록의 발견 지점이 된다(서비스 칩과 같은 철학).
///
/// 네 모드(`ScriptChipMode` 순수 판정): **빈 칩**=플레이스홀더(클릭=도크 스크립트 탭) /
...
struct ScriptStrip: View {
    ...
    /// 등록 0개 — 이름만 말하는 플레이스홀더(ServiceStrip.placeholder와 같은 문법). 클릭 = 도크 스크립트 탭.
    private var placeholder: some View {
        ...
        .buttonStyle(.plain)
        .clickCursor()
        .help("스크립트 — 빌드·테스트처럼 끝이 있는 명령. 클릭해 추가·실행")
        .accessibilityLabel("스크립트")
    }

    /// 평시 — 개수만 말하는 조용한 칩. 클릭 = 도크 스크립트 탭.
    ...
    private func runningChip(_ active: [ScriptRun]) -> some View {
        ...
        .accessibilityLabel("스크립트 \(latest.name) \(ScriptStatusStyle.label(latest.state)) — 클릭해 출력 보기")
    }
