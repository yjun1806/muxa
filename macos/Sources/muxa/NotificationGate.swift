...
/// 알림 배달 결정(순수 값) — 세 채널을 각각 켤지 끌지.
/// - `flashPane`: 보이는 칸 테두리 플래시("어느 칸이 울렸나"를 즉시 짚음).
...
struct NotificationDelivery: Equatable {
    ...
    /// 아무것도 안 함(보이는 칸의 유휴 리마인더처럼 완전 억제).
    static let suppressed = NotificationDelivery(flashPane: false, badge: false, systemNotification: false)
    /// 테두리 플래시만(보이는 칸 — 주의는 이미 사용자에게).
    static let flashOnly = NotificationDelivery(flashPane: true, badge: false, systemNotification: false)
    /// 배지만(안 보이지만 조용히 — 유휴 리마인더).
    static let badgeOnly = NotificationDelivery(flashPane: false, badge: true, systemNotification: false)
    /// 배지 + 시스템 알림(안 보이는 칸의 긴급/완료/자동 신호 — 기존 기본 동작).
    static let badgeAndNotify = NotificationDelivery(flashPane: false, badge: true, systemNotification: true)
}
...
///
/// 결정 원칙:
/// - **보이는 칸은 시스템 알림·배지 억제**(주의는 이미 사용자에게) — 테두리 플래시로만 짚는다.
///   단 `idle-reminder`는 보이면 완전 억제(사용자가 이미 보는 칸엔 유휴 알림이 불필요).
/// - 안 보이면 카테고리 긴급도로 분기: `needs-permission`·`turn-complete`·`nil`(자동 신호)은
///   배지 + 시스템 알림, `idle-reminder`는 배지만(조용히 — 억제 가능).
///
/// `category == nil`(OSC 9/777 등 자동 신호)은 **기존 desktopNotification 동작을 그대로** 보존한다
/// (보이면 플래시, 안 보이면 배지+시스템 알림) — 회귀 없음.
enum NotificationGate {
    static func shouldDeliver(category: NotifyCategory?, isVisibleToUser: Bool) -> NotificationDelivery {
        if isVisibleToUser {
            return category == .idleReminder ? .suppressed : .flashOnly
        }
        switch category {
        case .idleReminder:
            return .badgeOnly
