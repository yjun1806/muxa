...
/// 모서리 반경 — **두 단**이 전부다. 컨트롤이냐 떠 있는 면이냐, 그 한 가지만 가른다.
/// (예전엔 4·5·6·8·10이 흩어져 있었다 — 한 픽셀씩 다른 값은 눈엔 안 보이고 일관성만 깎아먹는다.
///  두 단으로 접고 값을 살짝 키워 재조정했다.)
enum Radius {
    /// 컨트롤·칩·탭·목록 행·작은 오버레이 — "손에 잡히는 작은 면"은 전부 이것.
    static let sm: CGFloat = 6
    /// 떠 있는 면·콘텐츠 카드·팝오버·명령 팔레트 — "위에 얹힌 큰 판"은 전부 이것.
...
enum IconSize {
    ...
    /// 프로젝트 행의 **상태 아이콘** 슬롯 — 점(6pt) 대신 글리프를 쓰되 과하지 않게(Orca식 상태 아이콘).
    /// 슬롯을 고정해야 아이콘 모양이 상태마다 달라도(링·원·느낌표) 이름이 흔들리지 않는다.
    static let statusGlyph: CGFloat = 11
    /// 신호 점(작업중·주의).
