...
struct AgentChangeSet: Equatable, Codable {
    ...
    /// **상세 탭이 열려 있었나** — 재시작 후 되살리기 위한 표식.
    ///
    /// 여기 두는 이유: 그룹 탭은 세션을 `TabID`로 가리키는데 그건 복원 때 재발급된다.
    /// 반면 이 파일의 키(지속 탭이면 tmux 세션명)는 **재시작을 그냥 넘긴다** —
    /// "어느 세션의 상세였나"를 안전하게 적어둘 수 있는 유일한 자리다.
    var detailWasOpen = false
