...
enum SessionAttachment: Equatable {
    /// 아무도 안 붙었다 — **이 창이 찾아야 할 상태**. 어느 화면에도 없이 살아 있는 세션이다.
    case detached
    /// 이 앱의 탭으로 보이고 있다.
    case thisApp
    ...
    /// 화면에 보이고 있는가 — 보이는 세션을 죽이면 누군가의 화면이 사라진다.
    var isVisible: Bool { self != .detached }
}
...
enum SessionOwnership {
    ...
    static func attachment(clientPids: [pid_t], snapshot: ProcessSnapshot,
        ...
        guard !clientPids.isEmpty else { return .detached }
