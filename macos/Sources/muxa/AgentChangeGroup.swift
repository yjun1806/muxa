...
enum AgentChangeDisplay {
    ...
    ///
    /// 폴더 순서는 **입력 순서를 그대로 따른다** — `group`이 이미 정렬해 놓은 것(안 본 것 먼저 →
    /// 최근순)의 첫 등장 순이다. 즉 "안 본 게 있는 폴더가 먼저, 그다음 최근"이지 순수 최근순이 아니다.
    /// 참고 목록(알파벳순)과 다른 이유: 참고는 시각을 안 보여주는 색인이라 안정성이 우선이고,
    /// 변경은 시각을 보여주는 작업 기록이라 손댄 순서가 정보다.
