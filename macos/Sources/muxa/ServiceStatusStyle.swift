...
/// 서비스 상태의 표시 규칙 — 색·글리프·꼬리표의 단일 출처(**서비스 축 SSOT**).
/// 푸터 칩·도크 목록·팝오버 세 곳이 같은 규칙을 써야 "빨간 건 죽은 것"이 흔들리지 않는다.
///
/// **에이전트 축(`StatusStyle`)과 통일하지 않는다 — 일부러 별개다.** 서비스 실행중(파랑 ▶)·정상종료(무채 ■)는
/// 에이전트 작업중(틸 ●)·완료(초록 ✓)와 색·모양이 달라야 한 사이드바 행에 나란히 떠도 안 헷갈린다(두 축 모델).
