...
struct SessionListItem: Identifiable, Equatable {
    ...
    /// **붙을 탭조차 없이 살아 있는 터미널** — 이 창의 표적.
    ///
    /// `미연결`(탭은 있는데 지금 안 붙음)은 여기 들지 않는다. 앱을 막 켰거나 사용자가 detach한
    /// 상태로, 그 탭으로 가면 다시 붙는다 — **잃어버린 세션이 아니다.** 그걸 섞으면 정상 상태가
    /// 목록을 채워 진짜 찾아야 할 것이 묻힌다.
    ///
    /// 스크립트·서비스도 제외한다: 화면에 없는 것이 그들의 정상이다(`new-session -d`).
